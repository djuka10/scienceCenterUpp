...
//	
	...
	private @ResponseBody List<MagazineDto> getAllMagazines(){
		List<Magazine> magazines = magazineRepo.findAll().stream().filter(m -> m.isActive()).collect(Collectors.toList());
		...
		for (Magazine magazine : magazines) {
			...
			for (ScienceArea scienceArea : areas) {
				areasDto.add(new ScienceAreaDto(scienceArea.getScienceAreaId(), scienceArea.getScienceAreaName(),scienceArea.getScienceAreaCode()));
			}
			
			magazinesDto.add(new MagazineDto(magazine.getMagazineId(), magazine.getUsername(), magazine.getName(), areasDto));
...
//			List<ScienceAreaDto> areasDto = new ArrayList<ScienceAreaDto>();
//			areas.forEach(a -> areasDto.add(new ScienceAreaDto(a.getId(), a.getScienceAreaName(), a.getScienceAreaCode())));
//			magazinesDto.add(new MagazineDto(m.getMagazineId(), m.getUsername(), m.getName(), areasDto));	
//		});
...
//	
	...
	private @ResponseBody MagazineDto getMagazine(@PathVariable Long magazineId){
		Magazine magazine = magazineRepo.getOne(magazineId);
		
		Set<ScienceArea> areas = magazine.getScienceAreas();
		List<ScienceAreaDto> areasDto = new ArrayList<ScienceAreaDto>();
		areas.forEach(a -> areasDto.add(new ScienceAreaDto(a.getScienceAreaId(), a.getScienceAreaName(), a.getScienceAreaCode())));
		
		return new MagazineDto(magazine.getMagazineId(), magazine.getUsername(), magazine.getName(), areasDto);		
	}
//		
