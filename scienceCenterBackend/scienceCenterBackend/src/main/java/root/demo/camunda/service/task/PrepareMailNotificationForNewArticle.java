...
import root.demo.model.EmailDto;
...
@Service
public class PrepareMailNotificationForNewArticle implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		EmailDto authorMail = new EmailDto(subjectAuthor, contentAuthor, "text/html", author.getMail(), sendFrom, false, "utf-8");
		EmailDto chiefEditorMail = new EmailDto(subjectEditor, contentEditor, "text/html", chiefEditor.getMail(), sendFrom, false, "utf-8");

		List<EmailDto> mailsToSend = new ArrayList<EmailDto>();
		mailsToSend.add(authorMail);
		mailsToSend.add(chiefEditorMail);
		
		execution.setVariable("mailNotificationsForNewArticle", mailsToSend);
	}
